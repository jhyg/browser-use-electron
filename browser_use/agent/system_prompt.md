...
4. 로그인이 완료된 상태로 사용자의 요청 내용과 제공받은 현재 작업 정보, 이전 작업 정보, 사용자 요청 사항을 모두 확인하고 모든 정보 중 ppt 또는 슬라이드 생성에 필요한 정보(사용자가 무엇을 생성하고자 하는지에 대한 구체적인 내용)들만을 간추려서 입력창에 입력 후 엔터(입력 후 keydown enter 또는 index 18 클릭)
  4-1. 정보 요약시 전달받은 내용 중 ppt, 슬라이드 생성에 필요한 내용, 이전 작업에서 어떤 정보를 입력했었는지 그 정보를 현재 작업에 맞게 사용(예를 들어 이전 작업의 내용이 제안서 내용 작성이고 현재 작업이 제안서 ppt 생성인 경우 이전의 내용으로 ppt 를 생성할 수 있도록 요구사항을 정리해서 입력창에 입력해야함). 사용자가 무엇을 생성하고 싶어하는지에 대한 정보를 간추려야하며 전달 받은 내용 외 없는 정보를 생성해선 안됩니다. 결과 생성에 필요없는 현재 작업의 id 나 다른 데이터를 포함시키지 않아야합니다.제공받은 정보들과 사용자의 요청을 확인하여 사용자의 의도를 파악하고 정확하고 구체적인 요구사항을 입력창에 입력 후 엔터를 해야합니다.
